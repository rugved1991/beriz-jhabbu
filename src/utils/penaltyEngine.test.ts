...
describe('PenaltyEngine', () => {
  ...
  describe('checkPenalty', () => {
    ...
    it('should select longest combination when both penalties apply', () => {
      const playedCard = createCard('6');
      const tableCards = [createCard('A'), createCard('2'), createCard('3'), createCard('6', 'diamonds')];
      // Duplicate rank: 6 + 6 = 2 cards total
      // Sum match: 6 + (A+2+3) = 4 cards total
      // Should pick sum match (longer)
      const result = checkPenalty(playedCard, tableCards);
      expect(result).not.toBe(null);
      expect(result?.length).toBe(4); // Played card + A + 2 + 3
      // Verify it contains the sum cards, not just the duplicate
      const ranks = result?.map(c => c.rank).sort();
      expect(ranks).toEqual(['2', '3', '6', 'A']);
    });

    it('should pick sum penalty over duplicate when sum has more cards', () => {
      const playedCard = createCard('3');
      const tableCards = [createCard('3', 'diamonds'), createCard('A'), createCard('2')];
      // Duplicate rank: 3 + 3 = 2 cards total
      // Sum match: 3 + (A+2) = 3 cards total
      // Should pick sum match (longer)
      const result = checkPenalty(playedCard, tableCards);
      expect(result).not.toBe(null);
      expect(result?.length).toBe(3); // Played card + A + 2
      const ranks = result?.map(c => c.rank).sort();
      expect(ranks).toEqual(['2', '3', 'A']);
    });

    it('should pick duplicate penalty when it has more cards than sum', () => {
      const playedCard = createCard('5');
      const tableCards = [createCard('5', 'diamonds'), createCard('5', 'hearts'), createCard('2'), createCard('3')];
      // Duplicate rank: 5 + 5 + 5 = 3 cards total
      // Sum match: 5 + (2+3) = 3 cards total
      // Should pick duplicate (same length, but duplicate is checked first)
      const result = checkPenalty(playedCard, tableCards);
      expect(result).not.toBe(null);
      expect(result?.length).toBe(3);
      // All should be 5s
      const allFives = result?.every(c => c.rank === '5');
